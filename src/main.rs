...
use crate::{
    import_flow::PendingRecords, master_key::KeyOsAppSeedSource, persist::KeystoreStore, state::AppState,
};
...
impl Approver for SlintApprover {
    async fn request(&self, req: ApprovalRequest) -> ApprovalDecision {
        ...
        if let Some(old) = self.pending_tx.lock().unwrap().replace(tx) {
            let _ = old.send(ApprovalDecision::Reject);
        }
    ...
    fn cancel_pending(&self) -> bool {
        let cancelled = self
            ...
            .unwrap()
            .take()
            .map(|tx| tx.send(ApprovalDecision::Reject).is_ok())
            .unwrap_or(false);
        if cancelled {
            let weak = self.ui_weak.clone();
            let _ = slint_keyos_platform::slint::invoke_from_event_loop(move || {
